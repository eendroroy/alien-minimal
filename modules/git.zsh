...
am_git_branch(){
  ...
  echo -ne "%B%F{$am_branch_color}${ref#refs/heads/}%f%b";
}
...
am_git_dirty(){
  ...
  [[ "$_mod" != "0" ]] && echo -n "%F{$am_dirty_color} ⭑%f";
  [[ "$_add" != "0" ]] && echo -n "%F{$am_dirty_color} +%f";
  [[ "$_del" != "0" ]] && echo -n "%F{$am_dirty_color} -%f";
  [[ "$_new" != "0" ]] && echo -n "%F{$am_dirty_color} ?%f";
...
am_git_left_right(){
  ...
  if [[ $(am_git_branch) != "detached" ]]; then
    ...
    [[ "$_pull" != "0" ]] && [[ "$_pull" != "" ]] && echo -n "%F{$am_left_right_color} ▼%f";
    [[ "$_push" != "0" ]] && [[ "$_push" != "" ]] && echo -n "%F{$am_left_right_color} ▲%f";
